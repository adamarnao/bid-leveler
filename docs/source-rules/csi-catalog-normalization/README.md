...
1. `CSI_CATALOG_NORMALIZATION_SPEC.md` - source-of-truth policy, source roles, normalized catalog model, Division 00 policy, runtime output policy, and anti-patterns.
2. `csiCatalogNormalizationSource.ts` - source-rule constants for source roles, development sources, normalization stages, review statuses, expected outputs, placement policy, and anti-patterns.
